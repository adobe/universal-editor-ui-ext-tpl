...
const extFolder = "src/universal-editor-ui-1";
...
describe("prototype", () => {
  test("exports a yeoman generator", () => {
    expect(UniversalEditorWebAssetsGenerator.prototype).toBeInstanceOf(Generator);
  });
});
...
function assertFiles(extensionManifest) {
  ...
  const rightPanelRails = extensionManifest.rightPanelRails || [];
  rightPanelRails.forEach((panel) => {
    const railFileName = panel.header.replace(/ /g, "") + "Rail";
    assert.file(`${webSrcFolder}/src/components/${railFileName}.js`);
  });
}
...
function assertCodeContent(extensionManifest) {
  ...
  rightPanelRails.forEach((panel) => {
    ...
    assert.fileContent(
      `${webSrcFolder}/src/components/${railFileName}.js`,
      `export default function ${railFileName} ()`
    );
    assert.fileContent(
      `${webSrcFolder}/src/components/App.js`,
...
describe("run", () => {
  ...
  test("test a generator invocation with custom code generation", async () => {
    ...
    await helpers
      .run(UniversalEditorWebAssetsGenerator)
      .withOptions(options)
      .inTmpDir((dir) => {
